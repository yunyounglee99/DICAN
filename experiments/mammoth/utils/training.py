    ...
    with track_system_stats(logger, device=args.device) as system_tracker:
        ...
        for cur_task in range(start_task, end_task):
            ...
            if not args.inference_only and args.n_epochs > 0:
                ...
                # =========================================================================
                # [수정됨] Base Task vs Incremental Task 에포크 분리 로직
                # =========================================================================
                if cur_task == 0:
                    real_epochs = args.n_epochs  # Base Session은 설정값(예: 50) 그대로 사용
                    logging.info(f"[Schedule] Task {cur_task} (Base): Training for {real_epochs} epochs.")
                else:
                    real_epochs = 10  # Incremental Session은 20으로 고정 (과적합 방지)
                    logging.info(f"[Schedule] Task {cur_task} (Incremental): Training for {real_epochs} epochs (Few-shot Adjustment).")
                # =========================================================================
                ...
                if not isinstance(dataset, GCLDataset):
                    n_iterations = real_epochs * len(train_loader) if model.args.fitting_mode == 'epochs' else model.args.n_iters
                ...
                while True:
                    ...
                    epoch += 1
                    if args.fitting_mode == 'epochs' and epoch >= real_epochs:
                        break        
                    ...
                    if args.eval_epochs is not None and (epoch > 0 or args.eval_epochs) and epoch % args.eval_epochs == 0 and epoch < real_epochs:
